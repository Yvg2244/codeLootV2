...
const DisplayPage = () => {
  const[{newRoom,outputStatus},dispatch]=useStateValue()
  const [joinRoomRequest, setJoinRoomRequest] = useState(false);
  
  
  return (
    <main className="w-[full] h-[auto] pt-[2rem] px-[2rem] bg-primary_gray flex flex-col items-center text-white">
      {
        <JoinRoomModal
      ...
      <div className="flex mt-[30px] gap-5">
        ...
        <div
          className="py-[6px] cursor-pointer px-[36px] text-[16px] font-medium font-inter tracking-wide rounded-md bg-primary_gray text-white border-2 border-white"
          onClick={() => {
            setJoinRoomRequest(!joinRoomRequest);
          }}
        >
